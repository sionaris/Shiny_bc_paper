...
server <- function(input, output, session) {
  ...
  # Import data type
  import_data_type = input$breast_cancer_new_prediction_type
  desired_treatment = input$breast_cancer_new_prediction_treatment
  file_input = input$breast_cancer_new_prediction_file_input
 ...
 # If import is chosen: Load the file
  observeEvent(input$import_new_prediction_breast_cancer, {
    ...
    if (grepl(".csv$", file_input$datapath)) {
      imported_data =  readr::read_csv(file_input$datapath, show_col_types = FALSE)
    } else if (grepl(".txt$", file_input$datapath)) {
      imported_data = data.table::fread(file_input$datapath)
    } else if (grepl(".xlsx$", file_input$datapath)) {
      imported_data = openxlsx::read.xlsx(file_input$datapth)
    } else if (grepl(".tsv$", file_input$datapath)) {
      imported_data = readr::read_tsv(file_input$datapath, show_col_types = FALSE)
    }
    
    full_colnames_imported_data = colnames(imported_data)
    ...
    if (is.null(imported_data)) {
      ...
      if (grepl("X_", colnames(imported_data))) {
        X_handle = "on"
        gene_colnames_imported_data = intersect(colnames(imported_data), gene_columns_with_X)
        X_columns_imported_number = length(gene_colnames_imported_data)
      } else {
        X_handle = "off"
        gene_colnames_imported_data = intersect(intersect(colnames(imported_data), gene_columns_without_X))
        nonX_gene_columns_imported_number = length(gene_colnames_imported_data)
      }
      ...
      if (X_handle == "on" && X_columns_imported_number < 166) {
        showModal(
          modalDialog(
            ...
            paste0("There are missing markers in the data. 166 variables required.",
                   "\n", "Problem with:", setdiff(gene_columns_with_X, gene_colnames_imported_data)), easyClose = TRUE 
          )
        )
        ...
        showModal(
          modalDialog(
            title = "Warning", 
            paste0("There are missing markers in the data. 166 variables required.",
                   "\n", "Problem with:", setdiff(gene_columns_without_X, gene_colnames_imported_data)), easyClose = TRUE 
          )
        )
      }
    }
    ...
    if (import_data_type == "Import unique sample (genes only)" && nrow(imported_data) > 1) {
      showModal(
        modalDialog(
          title = "Warning", 
    ...
    if (import_data_type == "Import unique sample (pre-annotated)" && nrow(imported_data) > 1) {
      showModal(
        modalDialog(
          title = "Warning", 
          "There are more than one rows in this sample. Only one-row files permitted.", easyClose = TRUE 
    ...
    if (import_data_type == "Import unique sample (genes only)") {
      ...
      if (any(!sapply(first_row, is.numeric))) {
        ...
        if (any(is.na(converted_row))) {
          ...
          imported_data[1, ] <- converted_row
        }
      }
    } else if (import_data_type == "Import unique sample (pre-annotated)" && X_handle == "on") {
      first_row <- imported_data[1, gene_columns_with_X]
      if (any(!sapply(first_row, is.numeric))) {
        # Attempt to convert non-numeric values to numeric
        converted_row <- suppressWarnings(as.numeric(first_row))
        if (any(is.na(converted_row))) {
          ...
          imported_data[1, gene_columns_with_X] <- converted_row
        }
      }
    } else if (import_data_type == "Import unique sample (pre-annotated)" && X_handle == "off") {
      first_row <- imported_data[1, gene_columns_without_X]
      if (any(!sapply(first_row, is.numeric))) {
        # Attempt to convert non-numeric values to numeric
        converted_row <- suppressWarnings(as.numeric(first_row))
        if (any(is.na(converted_row))) {
          ...
          imported_data[1, gene_columns_without_X] <- converted_row
        }
      }
    } else if (import_data_type == "Import pre-annotated dataset" && X_handle == "on") {
      if (any(!sapply(imported_data[, gene_columns_with_X], is.numeric))) {
        # Attempt to convert non-numeric values to numeric
        converted_data <- suppressWarnings(as.data.frame(lapply(imported_data[, gene_columns_with_X], as.numeric)))
        if (any(is.na(converted_data))) {
          ...
          imported_data[, gene_columns_with_X] <- converted_data
        }
      }
    } else if (import_data_type == "Import pre-annotated dataset" && X_handle == "off") {
      if (any(!sapply(imported_data[, gene_columns_without_X], is.numeric))) {
        # Attempt to convert non-numeric values to numeric
        converted_data <- suppressWarnings(as.data.frame(lapply(imported_data[, gene_columns_without_X], as.numeric)))
        if (any(is.na(converted_data))) {
          showModal(modalDialog(
            title = "Warning",
            "The uploaded file contains non-numeric values that could not be converted to numeric.",
          ...
          imported_data[, gene_columns_without_X] <- converted_data
        }
      }
    }
    ...
    if (import_data_type %in% c("Import unique sample (pre-annotated)",
      ...
      if (length(intersect(colnames(imported_data), all_correct_columns)) < 183) {
        showModal(
          modalDialog(
            title = "Warning", 
            paste0("There are missing columns in the data.",
                   "\n", "Problem with:", setdiff(all_correct_columns, colnames(imported_data))), easyClose = TRUE 
          )
        )
      }
    }
    ...
    if (import_data_type == "Import unique sample (pre-annotated)") {
      if (!all(imported_data[1, pheno_columns] %in% c(0, 1))) {
        showModal(
          modalDialog(
            title = "Warning", 
      ...
      if (!all(apply(imported_data[, pheno_columns], 2, function(col) all(col %in% c(0, 1))))) {
        showModal(
          modalDialog(
            title = "Warning", 
            "All values in the phenotypic column should be either 0 or 1.", easyClose = TRUE 
    ...
    if (import_data_type == "Import unique sample (pre-annotated)") {
      first_row <- imported_data[1, pheno_columns]
      if (any(!sapply(first_row, is.factor))) {
        ...
        if (any(is.na(converted_row))) {
          ...
          imported_data[1, pheno_columns] <- converted_row
        }
      }
    } else if (import_data_type == "Import pre-annotated dataset") {
      if (any(!sapply(imported_data[, pheno_columns], is.factor))) {
        # Attempt to convert non-numeric values to numeric
        converted_data <- suppressWarnings(as.data.frame(lapply(imported_data[, pheno_columns], 
                                                                factor(imported_data[, pheno_columns],
        ...
        if (any(is.na(converted_data))) {
          ...
          imported_data[, pheno_columns] <- converted_data
        }
      }
    }
    ...
    # pam50
    pam50_subtype_edit = input$breast_cancer_new_prediction_pam50_annotation
    if (pam50_subtype_edit != "Preset") {
      if (pam50_subtype_edit == "Random") {
        pam50s = c("Luminal A", "Luminal B", "Normal-like",
                   "Basal-like", "HER2+")
        pam50_subtype_edit = pam50s[sample(seq(1, 5), 1)]
      }
    ...
    timepoint_edit = input$breast_cancer_new_prediction_timepoint_annotation
    if (timepoint_edit != "Preset") {
      if (timepoint_edit == "Random") {
        timepoints = c("Pre-treatment", "On-treatment")
        timepoint_edit = timepoints[sample(seq(1, 2), 1)]
      }
